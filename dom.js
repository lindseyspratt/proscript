...
function predicate_dom_element_tag(element, tag) {
    var cursor;
    var cursorIDPL;
    var cursorIDJS;
    ...
    if (state.foreign_retry) {
        cursorIDPL = state.foreign_value;
        cursorIDJS = atable[VAL(cursorIDPL)];
        cursor = detCursors.get(cursorIDJS);

        debug_msg("Is retry! Setting cursor back to " + cursor);
    }
    else {
        cursor = {
            elements: setupElementsForTag(element, tag),
            tags: setupTags(element, tag)
        };
        cursorIDJS = 'crs' + detCursorCounter++;
        detCursors.set(cursorIDJS, cursor);
        cursorIDPL = lookup_atom(cursorIDJS);
    ...
    if (cursor.elements && cursor.elements.length > 0) {
        ...
        if (!cursor.tags) {
            cursor.tags = setupTagsFromJSElement(elementJS, tag);
        }

        if (cursor.tags && cursor.tags.length > 0) {
            var classJS = cursor.tags.pop();
            var classPL = lookup_atom(classJS);
            return unify(tag, classPL) &&
                unify(element, elementPL);
        }
...
function setupElementsForTag(element, tag) {
    if (TAG(element) !== TAG_REF) {
        if (TAG(element) !== TAG_STR) {
            instantiation_error(element);
        }
        ...
        elements.push(elementJS);
        return elements;
    } else if (TAG(tag) !== TAG_REF) {
        if (TAG(tag) !== TAG_ATM) {
            instantiation_error(tag);
        }

        var tagJS = atable[VAL(tag)];
        elements = Array.from(document.getElementsByTagName(tagJS));
        return elements;
    } else {
        return Array.from(document.querySelectorAll('*'));
    }
...
function setupTagsFromJSElement(elementJS, tag) {
    var tags = [];
    if (TAG(tag) !== TAG_REF) {
        if (TAG(tag) !== TAG_ATM) {
            instantiation_error(tag);
        ...
        tags.push(atable[VAL(tag)]);
    } else {
        /** @namespace elementJS.tagName */
        tags.push(elementJS.tagName);
    }
    return tags;
}

function setupTags(element, tag) {
    var tags = [];
    if (TAG(tag) !== TAG_REF) {
        if (TAG(tag) !== TAG_ATM) {
            instantiation_error(tag);
...
function predicate_set_dom_element_inner_html(element, html) {
    if (TAG(element) !== TAG_STR) {
        instantiation_error(element);
    }
    ...
    elementJS.innerHTML = codes_to_string(html);
    return true;
}

function predicate_dom_element_inner_html(element, html) {
    var cursor;
    var cursorIDPL;
    var cursorIDJS;
    ...
    if (state.foreign_retry) {
        cursorIDPL = state.foreign_value;
        cursorIDJS = atable[VAL(cursorIDPL)];
        cursor = deihCursors.get(cursorIDJS);

        debug_msg("Is retry! Setting cursor back to " + cursor);
    }
    else {
        cursor = {
            elements: setupElementsForHTML(element),
            html: setupHTML(element, html)
        };
        cursorIDJS = 'crs' + deihCursorCounter++;
        deihCursors.set(cursorIDJS, cursor);
        cursorIDPL = lookup_atom(cursorIDJS);
    ...
    if (cursor.elements && cursor.elements.length > 0) {
        ...
        if (!cursor.html) {
            cursor.html = setupHTMLFromJSElement(elementJS, html);
        }

        if (cursor.html && cursor.html.length > 0) {
            var htmlJS = cursor.html.pop();
            var htmlPL = string_to_codes(htmlJS);
            return unify(html, htmlPL) &&
                unify(element, elementPL);
        }

        // All html for current elementJS have been processed.
        // Set the cursor.html to undefined to force recalculation of html
        // with next element.
        // Move to the next element by removing elements[0].

        cursor.html = undefined;
        cursor.elements = cursor.elements.slice(1);
        return false; // go to next choice (of element)
    } else {
        destroy_choicepoint();
...
function setupElementsForHTML(element) {
    if (TAG(element) !== TAG_REF) {
        if (TAG(element) !== TAG_STR) {
            instantiation_error(element);
        }
...
function setupHTMLFromJSElement(elementJS, html) {
    var htmlResult = [];
    if (TAG(html) !== TAG_REF) {
        if (TAG(html) !== TAG_ATM) {
            instantiation_error(html);
        ...
        htmlResult.push(htmlJS);
    } else {
        /** @namespace elementJS.innerHTML */
        htmlResult.push(elementJS.innerHTML);
    }
    return htmlResult;
}

function setupHTML(element, html) {
    var htmlResult = [];
    if (TAG(html) !== TAG_REF) {
        if (TAG(html) !== TAG_ATM) {
            instantiation_error(html);
